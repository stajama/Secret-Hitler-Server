...
def policyPresidentSelect(request, id, passing1, passing2, discarded):
    ...
    logging.debug(f"Catastrophe: policyPresidentSelect: passing1 = {passing1}, passing2 = {passing2}, discarded = {discarded}")
...
def policyChancellorSelects(request, id, passCard, discard):
    ...
    logging.debug(f"Catastrophe: policyChancellorSelect: passCard = {passCard}, discard = {discard}")
...
def executiveExecutionOrder(request, id, selected):
    ...
    if unluckyOne.president:
        ...
        cyclePresident()
        logging.debug('CHECK PRESIDENT QUEUE FOR ZOMBIES')
...
def getStatus(id):
    ...
    logging.debug('Start of getStatus() possible error here.')
...
def isGameOver():
    ...
    if G.fasPolicyCount >= 6:
        logging.debug('Game over for fascist policy win. state should be 100')
    ...
    if G.fasPolicyCount >= 3 and findCurrentChancellor().role == "Hitler":
        logging.debug('Game over for fascist election win. state should be 100')
    ...
    if not Player.objects.get(pk=findPlayerID(role="Hitler")).isAlive:
        logging.debug('Game over for liberal "fuck Hitler" win. state should be 100')
        updateGStatusUpdate("The Liberals have achieved victory by assassinating Hitler.")
        updateGStatusID(100) # New status for Game Over.
        return True
    if G.libPolicyCount >= 5:
        logging.debug('Game over for liberal policy win. state should be 100')
...
def cyclePresident():
    ...
    logging.debug('start of cyclePresident()')
    # debug_log_dump(playerDB=True)
    ...
    special = hold[0][1]
    while not x.isAlive:
        logging.debug("cyclePresident(): someone is dead: " + f'{x.name} is not alive. Cycling to next presidential candidate.')
        hold.pop(0)
        x = Player.objects.get(pk=hold[0][0])
        special = hold[0][1]
        logging.debug("cyclePresident(): someone is dead2: " + f'{x.name} is the next candidate')
    x.president = True
    x.save()
    if not special:
